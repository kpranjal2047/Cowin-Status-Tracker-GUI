...
public class ErrorAlert extends JFXDialog {
    ...
    private static final Runnable runnableSound = (Runnable) Toolkit.getDefaultToolkit()
            .getDesktopProperty("win.sound.asterisk");
    private static final Image errorIcon = new Image(
            Objects.requireNonNull(ErrorAlert.class.getResourceAsStream("/images/Icon_Error.gif")), 50, 50, true, true);
    private static final Text errorText = new Text("Error!");
    private static final ImageView imageView = new ImageView(errorIcon);
    private static final Text bodyText = new Text();
