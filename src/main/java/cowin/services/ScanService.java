...
public class ScanService extends ScheduledService<List<Center>> {
     ...
     * @param pinOrDistId PIN code or district ID
     * @param age         Age of the person
     * @param vaccineName Required vaccine name
     * @param doseNumber  Required dose number
     * @param feeType     "Free" or "Paid"
     */
    public ScanService(final ScanType scanType, final int pinOrDistId, final int age, final String vaccineName,
            final String doseNumber, final String feeType) {
                ...
                if (entity != null) {
                    ...
                    for (final JsonElement c : centers) {
                        ...
                        for (final JsonElement s : sessions) {
                            ...
                            final int dose1Count = session.getAsJsonPrimitive("available_capacity_dose1").getAsInt();
                            final int dose2Count = session.getAsJsonPrimitive("available_capacity_dose2").getAsInt();
                                ...
                                switch (doseNumber) {
                                    ...
                                    case "Precaution dose":
                                        ...
                                        if (precautionDoseCount > 0 && (age == -1 || age >= minAge)) {
                                            final Center available_center = new Center(name, pinCode, minAge, null,
                                                    vaccine, sessionDate, dose1Count, dose2Count, precautionDoseCount,
                                                    fee);
                            ...
                            if (age == -1 || age >= minAge) {
                                final boolean allowAllAge = session.getAsJsonPrimitive("allow_all_age").getAsBoolean();
                                if (allowAllAge) {
                                    final Center available_center = new Center(name, pinCode, minAge, null, vaccine,
                                            sessionDate, dose1Count, dose2Count, precautionDoseCount, fee);
                                    out.add(available_center);
                                } else {
                                    final int maxAge = session.getAsJsonPrimitive("max_age_limit").getAsInt();
                                    if (age == -1 || age <= maxAge) {
                                        final Center available_center = new Center(name, pinCode, minAge, maxAge,
                                                vaccine, sessionDate, dose1Count, dose2Count, precautionDoseCount, fee);
                                        out.add(available_center);
                                    }
                                }
                            }
